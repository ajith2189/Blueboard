...
function TableRow({ user, onBlock }: { user: User; onBlock: (id: string) => void }) {
  const chipClass = user.is_blocked
    ? "bg-red-100 text-red-700 dark:bg-red-900/50 dark:text-red-300"
    : "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/50 dark:text-emerald-300";

  // Enforcing single line content with text-ellipsis and overflow-hidden on table cells
  const cellClass = "px-6 py-4 truncate overflow-hidden whitespace-nowrap";

  return (
    <tr className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
      <td className="px-6 py-4 flex items-center space-x-3">
        <img src={user.profile_picture_url || noProfileUrl} className="w-10 h-10 rounded-full flex-shrink-0" />
        <div className="truncate overflow-hidden"> {/* Ensure text wraps/truncates in the Name column */}
...
export default React.memo(TableRow); // <-- ADDED: Memoization for performance